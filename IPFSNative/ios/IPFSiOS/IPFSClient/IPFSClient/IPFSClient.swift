...
public final class IPFSClient: NSObject {
    ...
    private let fakeData = "The Sylo Protocol acts as the confidential networking layer for all integrated decentralised applications," +
    ...
    public func add(_ callBack: @escaping ((String?, String?) -> Void)) {
        ...
        if let dir = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first {
            ...
            do {
                try fakeData.write(to: fileURL, atomically: false, encoding: .utf8)
            }
            catch {
            ...
            do {
                ...
                try? api?.add(path, completionHandler: { nodes in
                    ...
                    if let multihash = nodes.first?.hash {
